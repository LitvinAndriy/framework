...
function proverka_loginizacii($cuk_login, $cuk_password){
if(empty($cuk_login)&&empty($cuk_password)){
 redirect('login_form.php');
}
}

function andrej_get_url(){
$link =  $_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI];
return $link;
}

function trans($word_word, $lang){
		$translit = array(
		...
		if($lang == 'en'){
				$word = strtr($word_word, $translit);
				return($word);
			}
		if($lang == 'ru'){
				$word = strtr($word_word, array_flip($translit)); // обратная транслитерация. Переменная $word получит значение 'прочее'
				return($word);
			}		 
			
	}

function rabota_s_magazom(){
	echo'<ul class="shop_button">
		<li><a href="index.php?creat_shop">Создать магазин</a></li>
		<li><a href="index.php?edit_shop">Редактировать магазин</a></li>
		<li><a href="index.php?delete_shop">Удалить магазин</a></li>
		</ul>';
		
		echo '<p><input type="submit" name="exit" value="exit"></p>';
		if(isset($_POST['exit'])){
			setcookie("login");
			setcookie("password");
			redirect($url);
...
function doing_shop($url_shop){
	$pieces = end(explode("?", $url_shop));

	
	
	if($pieces == 'creat_shop'){
	   if($_POST['creat_shop']=='creat_shop'){
			print_r($_POST);
			print_r($_FILES);
	   }
	   
	   echo'<form enctype="multipart/form-data" method="POST" action="index.php?creat_shop">
		<p><input type="text" name="text_shop"></p>
		<p><input type="file" name="file_shop"></p>
		<p><input type="submit" name="creat_shop" value="creat_shop"></p>	
	    </form>';
		print_r($_FILES['file_shop']);
		$tranr_en = trans($_POST['text_shop'], 'en');
		 if(!file_exists('magaz_nazva')){ 
				mkdir('magaz_nazva');} else {
				if(!file_exists('magaz_nazva/'.$tranr_en)){
				
				mkdir('magaz_nazva/'.$tranr_en);
				move_uploaded_file($_FILES['file_shop']['tmp_name'], 'magaz_nazva/'.$tranr_en.'/'.$tranr_en.'.jpg');
				$foto = 'magaz_nazva/'.$tranr_en.'/'.$tranr_en.'.jpg';
			
				$otv = mysql_query("INSERT INTO magazin (nazva_magazin, photo_magazina) VALUES ('".$tranr_en."','".$foto."')");
				
				}  else { if(!empty($_POST['text_shop'])){
						 $rand =rand(1, 100);
						$nazv_magaz = 'magaz_nazva/'.$tranr_en.$rand;
						mkdir($nazv_magaz);
						$foto_magaz = $nazv_magaz.'/'.$tranr_en.$rand.'.jpg';
						print_r($foto_magaz);
						move_uploaded_file($_FILES['file_shop']['tmp_name'], $foto_magaz);
						$otv = mysql_query("INSERT INTO magazin (nazva_magazin, photo_magazina) VALUES ('".$tranr_en.$rand."','".$foto_magaz."')");
						//$otvet = msql_query($zapros);
						}
		}
	...
	if($pieces == 'edit_shop'){
	echo'выводиться функционал редактирования магазина';
	$select = mysql_query("SELECT * FROM magazin");
			echo '<table border="1" align="center" cellpadding="10">
			<tr>
			 <td> id </td><td> Название магазина </td> <td> Фотография магазина </td><td> Редактировать </td>
			</tr>';
			while($row = mysql_fetch_array($select)){
			 echo'<tr>
					<td>'.$row['id'].'</td>';
					$tranr_ru = trans($row['nazva_magazin'], 'ru');
					echo'<td>'.$tranr_ru.'</td>
					<td align="center"><img src="'.$row['photo_magazina'].'" width="50" height="50" alt="lorem"></td>
					<td align="center"><a href="index.php?redakt_shop='.$row['id'].'">Редактировать</a></td>
					</tr>';
	...
	$pars = parse_url($url_shop, PHP_URL_QUERY );// Анализ вторй ступеньки юрла
	  $pars = explode("=", $pars);
	  $pars_url = reset($pars);// Анализ вторй ступеньки юрла
	  $pars_id = end($pars);// Анализ вторй ступеньки юрла
		
	// редактирование определенного магазина по id.
	if($pars_url == 'redakt_shop'){
					if(!empty($_POST['redakt_shop'])){
					$tranr_en = trans($_POST['text_shop'], 'en');
					$update = mysql_query("UPDATE magazin SET nazva_magazin  = '".$tranr_en."' where id = '".$pars_id."'");
					redirect('index.php?edit_shop'); 
					}
				 echo'<form enctype="multipart/form-data" method="POST" action="index.php?redakt_shop='.$pars_id.'">';
				$select = mysql_query("SELECT * FROM magazin where id = '".$pars_id."' ");
				while($row = mysql_fetch_array($select)){
				$tranr_ru = trans($row['nazva_magazin'], 'ru');
					echo'<p><input type="text" name="text_shop" value="'.$tranr_ru.'"></p>
					<img src="'.$row['photo_magazina'].'"/>';
 				}
			  
		
				echo '<p><input type="submit" name="redakt_shop" value="redakt_shop"></p>	
				</form>';
	}
	

	if($pieces == 'delete_shop'){
		
		if($_POST['delete_shop']=='delete_shop'){
			print_r($_POST);
			print_r($_FILES);
	   }
			$select = mysql_query("SELECT * FROM magazin");
			echo '<table border="1" align="center" cellpadding="10">
			<tr>
			 <td> id </td><td> Название магазина </td> <td> Фотография магазина </td>
			</tr>';
			while($row = mysql_fetch_array($select)){
			 echo'<tr>
					<td>'.$row['id'].'</td>
					<td>'.$row['nazva_magazin'].'</td>
					<td align="center"><img src="'.$row['photo_magazina'].'" width="50" height="50" alt="lorem"></td>
					</tr>';
			...
			echo '<table>';
			 ...
			 $select = mysql_query("SELECT * FROM magazin");
			echo'<form action="index.php?delete_shop" method="post">
			<select size="5" multiple name="select">';
			while($raw = mysql_fetch_array($select)){
			echo'<option value="'.$raw['nazva_magazin'].'">'.trans($raw['nazva_magazin'], 'ru').'</option>';
			}
			echo'</select>
			<p><input type="submit" name="add" value="delete_shop"></p>
			</form>';
			print_r($_POST['select']);
			if(isset($_POST['add'])){
			mysql_query('DELETE FROM magazin WHERE nazva_magazin="'.$_POST['select'].'"');
			
			 $dir = 'magaz_nazva/'.$_POST['select'];
				removeDirectory($dir);
				redirect($url); 
